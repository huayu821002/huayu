...
export default function CheckoutPage() {
  ...
  const handlePlaceOrder = async () => {
    ...
    clearCart()
    router.push('/checkout/confirmation?order=JH-TEST-123')
  }
                ...
                <div key={step} className="flex items-center">
                  <div className={cn(
                    ...
                    i + 1 <= currentStep 
                      ...
                      : 'bg-joy-gray-200 text-joy-gray-500'
                  )}>
                    {i + 1 < currentStep ? <Icons.Check size={18} /> : i + 1}
                  </div>
                  <span className={cn(
                    'ml-2 font-medium hidden sm:inline',
                    i + 1 <= currentStep ? 'text-joy-gray-900' : 'text-joy-gray-400'
                  )}>
                    {step}
                  </span>
                  {i < STEPS.length - 1 && (
                    <div className={cn(
                      ...
                      i + 1 < currentStep ? 'bg-joy-orange' : 'bg-joy-gray-200'
                    )} />
                  )}
                </div>
              ))}
                        ...
                        <div className="flex-1">
                          ...
                          <div className="flex items-center justify-between mt-2">
                            ...
                            <span className="font-semibold text-joy-orange">
                              {formatCurrency(convertPrice(item.product.price * item.quantity, currency), currency)}
                            </span>
                          </div>
                        </div>
                      </div>
                        ...
                        <label className="flex items-center justify-between p-4 border-2 border-joy-orange rounded-xl cursor-pointer bg-joy-orange/5">
                          <div className="flex items-center gap-3">
                            ...
                            <div>
                              ...
                              <p className="text-sm text-joy-gray-500">7-10 business days</p>
                            </div>
                          </div>
                          <span className="font-semibold text-joy-gray-900">
                            {subtotal >= 299 ? 'FREE' : '$12.99'}
                          </span>
                        </label>
                        <label className="flex items-center justify-between p-4 border-2 border-joy-gray-200 rounded-xl cursor-pointer hover:border-joy-orange transition-colors">
                          <div className="flex items-center gap-3">
                            ...
                            <div>
                              ...
                              <p className="text-sm text-joy-gray-500">3-5 business days</p>
                            </div>
                          </div>
                          <span className="font-semibold text-joy-gray-900">$24.99</span>
                        </label>
                      </div>
                    </div>
                  </div>
                  ...
                  <div className="space-y-4 mb-6">
                    ...
                    <label className="flex items-center gap-4 p-4 border-2 border-joy-gray-200 rounded-xl cursor-pointer hover:border-joy-orange transition-colors">
                      ...
                      <svg className="w-6 h-6" viewBox="0 0 24 24" fill="currentColor">
                        <path d="M7.076 21.337H2.47a.641.641 0 0 1-.633-.74L4.944.901C5.026.382 5.474 0 5.998 0h12.004c.524 0 .972.382 1.054.901l3.107 19.696a.641.641 0 0 1-.633.74h-4.606a.75.75 0 0 1-.612-.314l-1.937-2.754-1.937 2.754a.75.75 0 0 1-.612.314H7.076z"/>
                      </svg>
            ...
            <div className="lg:col-span-1">
              <div className="bg-white rounded-2xl shadow-sm p-6 sticky top-40">
                ...
                <div className="space-y-3 mb-6">
                  {items.slice(0, 3).map((item) => (
                    <div key={item.id} className="flex gap-3">
                      ...
                      <p className="text-sm font-semibold text-joy-gray-900">
                        {formatCurrency(convertPrice(item.product.price * item.quantity, currency), currency)}
                      </p>
                    </div>
                  ))}
                  {items.length > 3 && (
                    <p className="text-sm text-joy-gray-500 text-center">
                ...
                <div className="border-t border-joy-gray-100 pt-4 space-y-3">
                  ...
                  <div className="flex justify-between text-sm">
                    <span className="text-joy-gray-600">Tax (8%)</span>
                    <span className="font-medium">{formatCurrency(tax, currency)}</span>
                  </div>
                  <div className="flex justify-between text-lg font-bold pt-3 border-t border-joy-gray-100">
                    <span>Total</span>
                    <span className="text-joy-orange">{formatCurrency(total, currency)}</span>
                  </div>
                </div>
                ...
                <div className="mt-6">
                  ...
                  <select 
                    value={currency}
                    onChange={(e) => useCartStore.getState().setCurrency(e.target.value as Currency)}
